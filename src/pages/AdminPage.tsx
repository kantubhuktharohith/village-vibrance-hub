...
import { Plus, Check, X, Trash2, MapPin, Edit2, Shield, ArrowLeft } from "lucide-react";
...
const AdminPage = () => {
  ...
  const handleEdit = (place: TouristPlace) => {
    setForm({
      ...
      starting_price: String(place.starting_price),
  ...
  const handleVerify = async (id: string, verified: boolean) => {
    ...
    if (!error) {
      toast({ title: verified ? "Verified" : "Unverified", description: `Place ${verified ? "verified" : "unverified"} successfully` });
      fetchPlaces();
    }
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    if (!error) {
      toast({ title: "Deleted", description: "Tourist place removed" });
      fetchPlaces();
    }
  };
      ...
      <div className="sticky top-0 z-40 border-b bg-background/95 backdrop-blur-sm">
        <div className="flex items-center gap-3 px-5 py-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-muted-foreground">Manage tourist places</p>
          </div>
      ...
      <div className="px-5 py-4">
            ...
            >
              <div className="glass-card p-5 space-y-4">
                <h3 className="font-display text-lg font-semibold">
                  {editingId ? "Edit Place" : "Add New Tourist Place"}
                </h3>
        ...
        {loadingPlaces ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-muted-foreground">No tourist places yet. Add your first one!</p>
          </div>
        ) : (
            ...
            {places.map((place) => (
              ...
              >
                <div className="flex gap-4 p-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {place.starting_price > 0 && (
                      <p className="text-xs font-semibold mt-1">From ₹{place.starting_price}</p>
                    )}
                  </div>
                </div>
                <div className="flex border-t border-border">
                  ...
                  <button
                    onClick={() => handleVerify(place.id, !place.verified)}
                    className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 text-xs font-medium transition-colors border-x border-border ${
                      place.verified ? "text-destructive hover:bg-destructive/5" : "text-primary hover:bg-primary/5"
                    }`}
                  >
